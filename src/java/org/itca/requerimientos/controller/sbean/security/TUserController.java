...
import org.itca.requerimientos.controller.facade.security.TUserFacade;
...
public class TUserController implements Serializable {
    ...
    @EJB
    private org.itca.requerimientos.controller.facade.security.TUserFacade ejbFacade;
    ...
    public List<TRole> getUserRoles() {
        if (this.userRoles == null) {
            if (current == null) {
                this.userRoles = new ArrayList<TRole>();  // nueva lista si current es null
                return userRoles;
            }
            this.userRoles = ejbTUserRoleFacade.findRolesByUser(current.getUsername());  // asignar lista de objetos dependientes
        }
        return userRoles;
    }
    ...
    public String create() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/org/itca/requerimientos/bundles/SecurityBundle").getString("TUserCreated"));
    ...
    public String update() {
        try {
            ...
            for (TRole rol : this.userRoles) {
                System.out.println(this.ejbTUserRoleFacade.findByRolAndUser(rol.getRole(), current.getUsername()));
                if (this.ejbTUserRoleFacade.userHasRole(rol.getRole(), current.getUsername())) {
                    this.ejbTUserRoleFacade.create(new TUserRole(current.getUsername(), rol.getRole()));
                }
            }
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/org/itca/requerimientos/bundles/SecurityBundle").getString("TUserUpdated"));
            return "View";
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/org/itca/requerimientos/bundles/SecurityBundle").getString("PersistenceErrorOccured"));
            return null;
    ...
    private void performDestroy() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/org/itca/requerimientos/bundles/SecurityBundle").getString("TUserDeleted"));
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/org/itca/requerimientos/bundles/SecurityBundle").getString("PersistenceErrorOccured"));
        }
    }
